...
def ordenes(rutinaContable):
    ...
    print(rutinaContable)    
    print("-----------------Fin Registro diario------------------")

def rutinaContable(registroFacturas:list):
    registroDiario = {}
    registro = ""
    nuevoTotal = 0.0
    incremento = 25000
    for factura in registroFacturas:
        totalFactura = 0.0
    ...
    for orden,total in registroDiario.items():
        if total < 600000:
            ...
            registro += f"La factura {orden} tiene un total en pesos de {nuevoTotal:,.2f}\n"    
        else:
            registro += f"La factura {orden} tiene un total en pesos de {total:,.2f}\n"      
    return registro             
...
print(ordenes(rutinaContable(registroFacturas)))      
